...
for file in os.listdir(input_dir):
    ...
    for fold_idx in range(5):
        ...
        if training_data:
            ...
            # Loop through files in input directory again
            for file in os.listdir(input_dir):
                if file.endswith(".xml"):
                    xml_file = os.path.join(input_dir, file)
                    tree = ET.parse(xml_file)
...
# Output files paths
output_file_txt = os.path.join(output_dir, f"output_BILSTM_CE_CV_{current_datetime}_{hash_value}.txt")
output_file_html = os.path.join(output_dir, f"output_BILSTM_CE_CV_{current_datetime}_{hash_value}.html")
