...
# RNN Bidirectional LSTM Layer with word and char embeddings concatenated
        ...
        for sentence in root.iter('de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Sentence'):
            ...
            if sentence.find(".//webanno.custom.Judgmentsentity") is not None:
                ...
                if sentence_text not in sentences:
                    ...
                    for sent_idx, sent in enumerate(sentences_list[:10]):  # Select up to 5 sentences
...
# Output files paths
output_file_txt = os.path.join(output_dir, f"output_BILSTM_CE_{current_datetime}_{hash_value}.txt")
output_file_html = os.path.join(output_dir, f"output_BILSTM_CE_{current_datetime}_{hash_value}.html")
