                ...
                if sentence_text not in sentences:
                    ...
                    for sent_idx, sent in enumerate(sentences_list[:10]):  # Select up to 10 sentences
                        ...
                        for word in context_words:
                            ...
                            if word.lower() == annotated_word.lower():
                                word_embedding = np.ones_like(word_embedding)  # Set annotated word's embedding to 1.0
                        ...
                        num_samples = len(filtered_sentence_prediction_list)
                        # Reshape the input data
                        X = np.array(
                            [word_embedding.reshape((1, 100)) for word in context_words])
                        y = []
                        for word in context_words:
                            if word == annotated_word.lower():
                                y.append([1.0] + [0.0] * (num_classes - 1))
                                # One-hot encoded for the annotated word
                            else:
                                y.append(np.random.uniform(0.0, 0.1, num_classes))
                                # Random values for other words
